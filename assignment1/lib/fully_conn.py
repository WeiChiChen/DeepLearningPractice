...
class SmallFullyConnectedNetwork(object):
	def __init__(self, dropout_p=0, dtype=np.float32, seed=None):
		self.net = sequential(
			########## TODO: ##########
			# input is 4, hidden is 30, output is 7
			# init scale should also design by input std, now is 0.02
...
class TinyNet(object):
	def __init__(self, dropout_p=0, dtype=np.float32, seed=None):
		""" Use two conv layers, and put relu between them to make sure system
			is non-linear.
		"""
		self.net = sequential(
			########## TODO: ##########
