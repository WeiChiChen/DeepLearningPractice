...
def compute_acc(model, data, labels, num_samples=None, batch_size=100):
	...
	for i in range(num_batches):
		...
		# pred get the classification index of every img
		preds.append(pred)
...
"""
	Make sure the training iteration part work,
	go forward to get loss, go backward to get gradients
	then optimize it.
"""
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in range(max_epochs):
		...
		#############################################################################
		train_acc = compute_acc(model, data_batch, labels_batch,batch_size=batch_size)  # num_samples=1000
		...
		if val_acc > opt_val_acc:
			...
			for k in model.net.params:
				#print(k)
				opt_params[k] = model.net.params[k].copy()
				#print(opt_params[k])
			#print(opt_params)
			#############################################################################
			#                             END OF YOUR CODE                              #
